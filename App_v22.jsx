    ...
    <div style={{ height:"100vh",background:C.bgPrimary,fontFamily:font,
      ...
      {listening && (
        <div style={{ padding:"6px 16px", background:C.bgPrimary,
          ...
          <span style={{ color:C.terra, fontSize:"10px", fontStyle:"italic" }}>Listening...</span>
  ...
  const postPrayer = async () => {
    ...
    try {
      const r = await fetch("/api/sync?action=prayer_post", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: text.trim().slice(0, MAX_CHARS) })
      });
      const d = await r.json();
...
useEffect(() => {
  ...
  useEffect(() => {
    saveToStorage({
      ...
      lastActiveDate, bestStreak, totalActiveDays, graceUsedWeek, sessionHistory, assessmentResults, userEmail, stripeEmail, seasonalMode, benchItems,
    });
