...
// ═══════════════════════════════════════════════════════
// EMAIL GATE — mandatory sign-up before trial starts
// ═══════════════════════════════════════════════════════
function EmailGateScreen({ onVerified, C, font }) {
  ...
  const verifyCode = async () => {
    ...
    try {
      const now = Date.now();
      const r = await fetch("/api/auth", { method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ action:"verify-code", email: email.trim().toLowerCase(), code: code.trim(), trialStart: now }) });
      const d = await r.json();
      if (d.success) {
        onVerified({ token: d.token, email: d.email, trialStart: d.trialStart || now });
      } else { setError("Incorrect or expired code. Try again."); }
    } catch { setError("Network error. Please try again."); }
    setLoading(false);
  };
    ...
    <div style={{ minHeight:"100vh", background:bg, display:"flex", flexDirection:"column",
      ...
      <div style={{ maxWidth:"400px", width:"100%", opacity:visible?1:0,
        ...
        <div style={{ textAlign:"center", marginBottom:"32px" }}>
          <WaveLogo size={38} color={sage}/>
          <h1 style={{ color:textDark, fontSize:"28px", fontWeight:"normal",
            letterSpacing:"2px", margin:"16px 0 6px" }}>selah</h1>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"10px", marginBottom:"14px" }}>
            <div style={{ width:"32px", height:"1px", background:sage, opacity:0.4 }}/>
            <span style={{ color:textMuted, fontSize:"10px", letterSpacing:"4px",
              textTransform:"uppercase", fontStyle:"italic" }}>be still & know</span>
            <div style={{ width:"32px", height:"1px", background:sage, opacity:0.4 }}/>
          </div>
          <p style={{ color:textSoft, fontSize:"14px", fontStyle:"italic", lineHeight:"1.8",
            margin:"0", maxWidth:"300px", marginLeft:"auto", marginRight:"auto" }}>
            {step === "email"
              ? "Start your 7-day free trial. No payment needed — just your email."
              : `We sent a 6-digit code to ${email.trim().toLowerCase()}. Check your inbox.`}
          </p>
        </div>
        ...
        <div style={{ background:"#fff", borderRadius:"14px", padding:"28px 24px",
          ...
          {step === "email" ? (
            <>
              <label style={{ display:"block", color:textSoft, fontSize:"11px",
                letterSpacing:"2px", textTransform:"uppercase", marginBottom:"8px" }}>
                Your Email
              </label>
              <input
                type="email" value={email}
                onChange={e=>{ setEmail(e.target.value); setError(""); setSoftMsg(false); }}
              ...
              />
              {softMsg && !error && (
                <p style={{ color:textMuted, fontSize:"12px", fontStyle:"italic",
                  margin:"10px 0 0", textAlign:"center" }}>
                  Sign up to begin your free trial — it only takes a moment.
                </p>
              )}
        ...
        <p style={{ textAlign:"center", color:textMuted, fontSize:"11px",
          fontStyle:"italic", marginTop:"20px", lineHeight:"1.7" }}>
          No credit card required. Cancel anytime.<br/>
          Already have an account? Sign in from Settings after you're in.
        </p>
      </div>
    </div>
  );
...
function HomeScreen({ C, font, setScreen, userName, steadyDays, showLateNight, sharingEnabled, onboardingAnswers, faithLevel, isFirstVisit, onDismissWelcome, onLogMood, onActive, lastFeedbackPrompt, onDismissFeedback, sessionCount, tone, quoteFreq, tier, isTrialActive, onUpgrade, moodHistory, sessionHistory, journalEntries, setJournalEntries, seasonalMode, setSeasonalMode, currentSeason, graceUsedWeek, showTutorial, setShowTutorial }) {
  ...
  const [whatsNewDismissed,setWhatsNewDismissed]=useState(()=>{ try{ return localStorage.getItem("selah_whatsnew_v1")==="dismissed"; }catch{return false;} });
...
{"quote":"a meaningful quote (Bible verse, philosopher, psychiatrist, or faith leader)","author":"source or name","type":"Bible|Wisdom|Faith|Philosophy","theme":"the core theme","question":"one grounding reflection question tied to the quote — direct, not therapy-speak. Under 20 words.","action":"one small, concrete action they can take today. Under 20 words. Starts with a verb."}`}] }) });
      ...
      const t=d.content.map(b=>b.text||"").join("").trim();
      var aParsed=JSON.parse(t.replace(/```json|```/g,"").trim()); setQuote(aParsed); try{localStorage.setItem("selah_cached_anchor",JSON.stringify({quote:aParsed,timestamp:Date.now()}));const ac=parseInt(localStorage.getItem("selah_anchor_count")||"0");localStorage.setItem("selah_anchor_count",String(ac+1));}catch{}
    ...
    <div style={{ minHeight:"100vh",background:C.bgPrimary,fontFamily:font,
      ...
      {currentSeason && currentSeason.id !== "ordinary" && !seasonalMode && !seasonOptInDismissed && (TIER_LEVELS[tier]||0) >= TIER_LEVELS.foundation && (
        <div style={{ background:`${C.accent}08`, border:`1.5px solid ${C.accent}33`,
          ...
          <div style={{ display:"flex", gap:"10px" }}>
            <button onClick={()=>{ setSeasonalMode(true); try{localStorage.setItem("selah_season_optin_dismissed", new Date().toDateString());}catch{} setSeasonOptInDismissed(true); }} style={{
              background:C.accent, border:"none", borderRadius:"3px",
      ...
      {dailyEncouragement && (TIER_LEVELS[tier]||0) >= TIER_LEVELS.growth && (
        ...
        {/* ── WHAT'S NEW BANNER — moved to notifications panel ── */}
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",
          ...
          <div style={{ width:"100%",maxWidth:"380px",height:"100vh",
            ...
            <div style={{ padding:"12px 16px" }}>
              ...
              {!whatsNewDismissed && (
                <div style={{ background:`${C.accent}08`, border:`1.5px solid ${C.accent}33`,
                  borderRadius:"10px", padding:"16px 16px 12px", marginBottom:"12px",
                  position:"relative" }}>
                  <button onClick={()=>{ try{localStorage.setItem("selah_whatsnew_v1","dismissed");}catch{} setWhatsNewDismissed(true); }}
                    style={{ position:"absolute", top:"10px", right:"12px", background:"none",
                      border:"none", cursor:"pointer", color:C.textMuted, fontSize:"16px", lineHeight:1, padding:"2px 6px" }}>✕</button>
                  <p style={{ color:C.accent, fontSize:"9px", letterSpacing:"2.5px",
                    textTransform:"uppercase", margin:"0 0 6px", fontStyle:"italic" }}>✦ What's New in Selah</p>
                  <p style={{ color:C.textPrimary, fontSize:"13px", fontFamily:font,
                    fontWeight:"normal", margin:"0 0 10px", lineHeight:"1.5", paddingRight:"20px" }}>
                    A lot has been added. Here's what to explore.
                  </p>
                  <div style={{ display:"flex", flexDirection:"column", gap:"6px", marginBottom:"10px" }}>
                    {[
                      { icon:"✉️", name:"Letters to God", desc:"A private prayer journal. Just you and God." },
                      { icon:"⚔️", name:"Armor Up", desc:"5-minute morning routine — verse, breathe, intention." },
                      ...
                      <div key={f.name} style={{ display:"flex", alignItems:"flex-start", gap:"10px" }}>
                        ...
                        <p style={{ color:C.textSoft, fontSize:"11px", fontStyle:"italic",
                          lineHeight:"1.7", margin:0 }}>
                          <span style={{ color:C.textPrimary, fontStyle:"normal" }}>{f.name}</span> — {f.desc}
                        </p>
                      </div>
                    ))}
                  </div>
                  <button onClick={()=>{ try{localStorage.setItem("selah_whatsnew_v1","dismissed");}catch{} setWhatsNewDismissed(true); }}
                    style={{ background:"none", border:"none", cursor:"pointer",
                      color:C.accent, fontSize:"10px", letterSpacing:"2px",
                      textTransform:"uppercase", fontStyle:"italic", padding:"2px 0",
                      fontFamily:font }}>Got it →</button>
                </div>
              )}
...
const MILESTONES=[
  ...
  {id:"checkin_10",icon:"🔍",label:"Checking In",desc:"Quick Check-in used 10 times — you know how to pause",scripture:'"Let a man examine himself."',ref:"1 Cor. 11:28"},
  {id:"sessions_25",icon:"🧠",label:"Deep Thinker",desc:"25 sessions — you go where most won't",scripture:'"Wisdom is the principal thing."',ref:"Prov. 4:7"},
  {id:"journal_10",icon:"🖊️",label:"Inner Witness",desc:"10 journal entries — your story matters",scripture:'"These words I have spoken to you."',ref:"John 15:11"},
  {id:"streak_30",icon:"⭐",label:"One Month In",desc:"30 consecutive days of showing up",scripture:'"I can do all things through Christ."',ref:"Phil. 4:13"},
  {id:"biblical_3",icon:"📜",label:"Stories in the Bones",desc:"Completed 3 Biblical Reflections — their stories became yours",scripture:'"These things were written for our instruction."',ref:"Romans 15:4"},
  {id:"mood_25",icon:"🌊",label:"Emotional Fluency",desc:"Logged your mood 25 times — you speak your own language now",scripture:'"Out of the abundance of the heart the mouth speaks."',ref:"Luke 6:45"},
  {id:"journal_20",icon:"📕",label:"Storyteller",desc:"20 entries — you're writing your becoming",scripture:'"My tongue is the pen of a skillful writer."',ref:"Psalm 45:1"},
  {id:"prayer_posted",icon:"🕯️",label:"Lifted It Up",desc:"Posted to the Prayer Wall — you didn't carry it alone",scripture:'"Cast all your anxiety on him."',ref:"1 Peter 5:7"},
  {id:"prayer_supported",icon:"🤝",label:"Held Someone",desc:"Supported another on the Prayer Wall — you became an answer",scripture:'"Bear one another\'s burdens."',ref:"Gal. 6:2"},
  {id:"sessions_50",icon:"🏔️",label:"The Long Climb",desc:"50 reflections — you've done the real work",scripture:'"Those who hope in the Lord will renew their strength."',ref:"Isaiah 40:31"},
  {id:"streak_60",icon:"🌿",label:"Rooted",desc:"60-day streak — this is who you are now",scripture:'"Like a tree planted by streams of water."',ref:"Psalm 1:3"},
  {id:"sessions_100",icon:"🏛️",label:"A Hundred Rooms",desc:"100 reflection sessions — you've built something lasting",scripture:'"The wise man built his house on the rock."',ref:"Matthew 7:24"},
  {id:"streak_90",icon:"✦",label:"A Full Season",desc:"90 days — you've grown through something",scripture:'"Let us not grow weary in doing good."',ref:"Gal. 6:9"},
  {id:"journal_50",icon:"📚",label:"The Archive",desc:"50 entries — a record of your transformation",scripture:'"I will remember the deeds of the Lord."',ref:"Psalm 77:11"},
  {id:"streak_180",icon:"🌙",label:"Half a Year",desc:"180 days straight — you are not who you were",scripture:'"Do not conform to the pattern of this world, but be transformed."',ref:"Romans 12:2"},
...
function ProgressScreen({ C, font, setScreen, steadyDays, sessionCount, moodHistory, bestStreak, totalActiveDays, sessionHistory, journalEntries, tier, onUpgrade }) {
  const [unlocked]=useState(() => {
    ...
    if (breatheCount >= 5) u.push("breathe_5");
    if (sessionCount >= 10) u.push("sessions_10");
    if (steadyDays >= 14) u.push("streak_14");
    if (moodHistory && moodHistory.length >= 10) u.push("mood_10");
    if (checkinCount >= 10) u.push("checkin_10");
    if (sessionCount >= 25) u.push("sessions_25");
    if (journalEntries && journalEntries.length >= 10) u.push("journal_10");
    if (steadyDays >= 30) u.push("streak_30");
    if (biblicalDone.length >= 3) u.push("biblical_3");
    if (moodHistory && moodHistory.length >= 25) u.push("mood_25");
    if (journalEntries && journalEntries.length >= 20) u.push("journal_20");
    if (localStorage.getItem("selah_prayer_posted")) u.push("prayer_posted");
    if (localStorage.getItem("selah_prayer_supported")) u.push("prayer_supported");
    if (sessionCount >= 50) u.push("sessions_50");
    if (steadyDays >= 60) u.push("streak_60");
    if (sessionCount >= 75) u.push("sessions_75");
    if (sessionCount >= 100) u.push("sessions_100");
    if (steadyDays >= 90) u.push("streak_90");
    if (journalEntries && journalEntries.length >= 50) u.push("journal_50");
    if (steadyDays >= 180) u.push("streak_180");
...
// ═══════════════════════════════════════════════════════
// GRATITUDE PRACTICE SCREEN
// ═══════════════════════════════════════════════════════
function GratitudeScreen({ C, font, setScreen, userName, gratitudeLog, setGratitudeLog }) {
  const [entries, setEntries] = useState(["","",""]);
  const [submitted, setSubmitted] = useState(false);
  const [viewPast, setViewPast] = useState(false);

  const todayKey = new Date().toDateString();
  const todayLog = (gratitudeLog||[]).find(l => l.date === todayKey);

  const PROMPTS = [
    ["Something that made you smile today","Someone who showed up for you","One thing your body did right today"],
    ["A small comfort you almost overlooked","Someone you're grateful to have in your life","Something hard that taught you something"],
    ["A moment of peace today","Something in nature you noticed","One door that opened recently"],
    ["A memory that still warms you","Someone who prayed for you","Something you take for granted but shouldn't"],
    ["A challenge that made you stronger","A friend you can call anytime","Something beautiful you saw this week"],
    ["Food on your table","A Scripture that's carried you","One thing going right"],
    ["Rest you were given","A kindness shown to you","Something only God could have arranged"],
  ];
  const todayPrompts = PROMPTS[new Date().getDay()];

  const handleSubmit = () => {
    if (entries.filter(e=>e.trim()).length < 1) return;
    const entry = { date: todayKey, items: entries.filter(e=>e.trim()), timestamp: Date.now() };
    setGratitudeLog(prev => [entry, ...(prev||[]).filter(l=>l.date!==todayKey)]);
    setSubmitted(true);
  };
  ...
  if (viewPast) return (
    <div style={{ minHeight:"100vh", background:C.bgPrimary, fontFamily:font, padding:"40px 20px 100px", boxSizing:"border-box" }}>
      <div style={{ maxWidth:"480px", margin:"0 auto" }}>
        <button onClick={()=>setViewPast(false)} style={{ background:"none",border:"none",cursor:"pointer",color:C.textMuted,fontSize:"20px",marginBottom:"20px" }}>←</button>
        <Label text="Past Gratitude" color={C.amber} font={font}/>
        {(!gratitudeLog||gratitudeLog.length===0) ? (
          <p style={{ color:C.textMuted,fontSize:"13px",fontStyle:"italic",textAlign:"center",marginTop:"40px" }}>No entries yet.</p>
        ) : (gratitudeLog||[]).map((log,i)=>(
          <div key={i} style={{ background:C.bgSecondary,borderRadius:"10px",padding:"16px 18px",marginBottom:"10px",border:`1px solid ${C.border}` }}>
            <p style={{ color:C.amber,fontSize:"9px",letterSpacing:"2px",textTransform:"uppercase",fontStyle:"italic",margin:"0 0 8px" }}>{formatDate(log.date)}</p>
            {log.items.map((item,j)=>(
              <p key={j} style={{ color:C.textSoft,fontSize:"13px",fontStyle:"italic",lineHeight:"1.8",margin:"0 0 4px" }}>🙏 {item}</p>
            ))}
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bgPrimary, fontFamily:font, padding:"40px 20px 100px", boxSizing:"border-box" }}>
      <div style={{ maxWidth:"480px", margin:"0 auto" }}>
        <button onClick={()=>setScreen("home")} style={{ background:"none",border:"none",cursor:"pointer",color:C.textMuted,fontSize:"20px",marginBottom:"20px" }}>←</button>
        <Label text="Gratitude" color={C.amber} font={font}/>
        <h1 style={{ color:C.textPrimary,fontSize:"clamp(24px,6vw,32px)",fontWeight:"normal",margin:"0 0 6px" }}>Count your blessings.</h1>
        <p style={{ color:C.textSoft,fontSize:"12px",fontStyle:"italic",lineHeight:"1.8",margin:"0 0 28px" }}>
          Three things. Every day. It rewires how you see the world.
        </p>

        {submitted || todayLog ? (
          <div style={{ background:`${C.amber}08`,border:`1px solid ${C.amber}33`,borderRadius:"12px",padding:"24px 20px",textAlign:"center",marginBottom:"20px" }}>
            <p style={{ fontSize:"28px",margin:"0 0 10px" }}>🙏</p>
            <p style={{ color:C.amber,fontSize:"11px",letterSpacing:"2px",textTransform:"uppercase",fontStyle:"italic",margin:"0 0 8px" }}>Logged for today</p>
            <div style={{ textAlign:"left",margin:"12px 0" }}>
              {(submitted ? entries.filter(e=>e.trim()) : todayLog.items).map((item,i)=>(
                <p key={i} style={{ color:C.textSoft,fontSize:"13px",fontStyle:"italic",lineHeight:"1.8",margin:"0 0 4px" }}>✦ {item}</p>
              ))}
            </div>
          ...
          <div style={{ marginBottom:"20px" }}>
            {[0,1,2].map(i=>(
              <div key={i} style={{ marginBottom:"12px" }}>
                <p style={{ color:C.textMuted,fontSize:"11px",fontStyle:"italic",margin:"0 0 6px",letterSpacing:"0.5px" }}>{todayPrompts[i]}</p>
                <textarea
                  value={entries[i]}
                  onChange={e=>{ const n=[...entries]; n[i]=e.target.value; setEntries(n); }}
                  placeholder="Write anything..."
                  rows={2}
                  style={{ width:"100%",background:C.bgSecondary,border:`1.5px solid ${entries[i]?C.amber+"55":C.border}`,
                    borderRadius:"8px",padding:"12px 14px",color:C.textPrimary,fontSize:"13px",
                    fontStyle:"italic",fontFamily:font,lineHeight:"1.7",resize:"none",
                    boxSizing:"border-box",outline:"none",transition:"border-color 0.2s ease" }}/>
              </div>
            ))}
            <button onClick={handleSubmit} disabled={entries.filter(e=>e.trim()).length<1}
              style={{ width:"100%",background:entries.filter(e=>e.trim()).length>0?C.amber:"transparent",
                border:`1.5px solid ${entries.filter(e=>e.trim()).length>0?C.amber:C.border}`,
                borderRadius:"4px",color:entries.filter(e=>e.trim()).length>0?"#0D0B09":C.textMuted,
                fontFamily:font,fontSize:"10px",letterSpacing:"3px",textTransform:"uppercase",
                padding:"15px",cursor:entries.filter(e=>e.trim()).length>0?"pointer":"default",
                transition:"all 0.2s ease" }}>Give Thanks</button>
          </div>
        )}

        {(gratitudeLog||[]).length > 0 && (
          <button onClick={()=>setViewPast(true)} style={{ background:"none",border:"none",cursor:"pointer",
            color:C.textMuted,fontSize:"11px",fontStyle:"italic",letterSpacing:"1px",
            textDecoration:"underline",padding:0,fontFamily:font }}>
            View past entries ({gratitudeLog.length})
          </button>
        )}
      </div>
    </div>
...
// WIND DOWN SCREEN
// ═══════════════════════════════════════════════════════
function WindDownScreen({ C, font, setScreen, faithLevel, userName, tier, isTrialActive, onUpgrade }) {
  const [step, setStep] = useState(0); // 0=intro, 1=release, 2=breathe, 3=scripture, 4=prayer, 5=done
  ...
  const VERSES = [
    ...
    { text:"I will lie down and sleep in peace, for you alone, Lord, make me dwell in safety.", ref:"Psalm 4:8" },
  ...
  const runBreathe = (count, phase) => {
    const durations = { inhale:4000, hold:4000, exhale:6000 };
    const next = { inhale:"hold", hold:"exhale", exhale:"inhale" };
    setBreathePhase(phase);
    const t = setTimeout(()=>{
      const newCount = phase === "exhale" ? count+1 : count;
      if (newCount >= 4) { setBreatheActive(false); setBreatheCount(4); return; }
      runBreathe(newCount, next[phase]);
    }, durations[phase]);
    setTimerRef(t);
  };

  const phaseLabel = { inhale:"Breathe in...", hold:"Hold...", exhale:"Let go..." };
  const phaseDuration = { inhale:"4 sec", hold:"4 sec", exhale:"6 sec" };

  const steps = [
    // Intro
    <div key="intro" style={{ textAlign:"center", padding:"20px 0" }}>
      <p style={{ fontSize:"36px", margin:"0 0 16px" }}>🌙</p>
      <h2 style={{ color:C.textPrimary, fontSize:"22px", fontWeight:"normal", margin:"0 0 10px" }}>Wind Down</h2>
      <p style={{ color:C.textSoft, fontSize:"13px", fontStyle:"italic", lineHeight:"1.9", margin:"0 0 28px" }}>
        A short routine to release the day and prepare for rest. Takes about 5 minutes.
      </p>
      <button onClick={()=>setStep(1)} style={{ background:C.accent,border:"none",borderRadius:"4px",
        color:"#fff",fontFamily:font,fontSize:"10px",letterSpacing:"3px",textTransform:"uppercase",
        padding:"15px 32px",cursor:"pointer" }}>Begin</button>
    </div>,

    // Release
    <div key="release">
      <p style={{ color:C.accent,fontSize:"9px",letterSpacing:"3px",textTransform:"uppercase",fontStyle:"italic",margin:"0 0 16px" }}>Step 1 of 4 · Release</p>
      <h2 style={{ color:C.textPrimary,fontSize:"20px",fontWeight:"normal",margin:"0 0 10px" }}>What are you carrying?</h2>
      <p style={{ color:C.textSoft,fontSize:"13px",fontStyle:"italic",lineHeight:"1.8",margin:"0 0 16px" }}>
        Write down anything weighing on you — a worry, a conversation, something unfinished. Getting it out of your head helps your brain let go.
      </p>
      <textarea
        value={releaseText}
        onChange={e=>setReleaseText(e.target.value)}
        placeholder="Whatever's still on your mind..."
        rows={4}
        style={{ width:"100%",background:C.bgSecondary,border:`1.5px solid ${releaseText?C.accent+"55":C.border}`,
          borderRadius:"8px",padding:"12px 14px",color:C.textPrimary,fontSize:"13px",
          fontStyle:"italic",fontFamily:font,lineHeight:"1.7",resize:"none",
          boxSizing:"border-box",outline:"none",marginBottom:"16px" }}/>
      <p style={{ color:C.textMuted,fontSize:"11px",fontStyle:"italic",lineHeight:"1.7",margin:"0 0 20px" }}>
        "Cast all your anxiety on him because he cares for you." — 1 Peter 5:7
      </p>
      <button onClick={()=>setStep(2)} style={{ width:"100%",background:C.accent,border:"none",borderRadius:"4px",
        color:"#fff",fontFamily:font,fontSize:"10px",letterSpacing:"3px",textTransform:"uppercase",
        padding:"15px",cursor:"pointer" }}>I've released it →</button>
    </div>,

    // Breathe
    <div key="breathe" style={{ textAlign:"center" }}>
      <p style={{ color:C.accent,fontSize:"9px",letterSpacing:"3px",textTransform:"uppercase",fontStyle:"italic",margin:"0 0 16px" }}>Step 2 of 4 · Breathe</p>
      <h2 style={{ color:C.textPrimary,fontSize:"20px",fontWeight:"normal",margin:"0 0 10px" }}>4-4-6 Breathing</h2>
      <p style={{ color:C.textSoft,fontSize:"13px",fontStyle:"italic",lineHeight:"1.8",margin:"0 0 24px" }}>
        This pattern activates your nervous system's rest response. 4 cycles.
      </p>
      {!breatheActive && breatheCount === 0 && (
        <button onClick={startBreathe} style={{ background:C.accent,border:"none",borderRadius:"50%",
          width:"100px",height:"100px",cursor:"pointer",color:"#fff",fontSize:"11px",
          letterSpacing:"2px",textTransform:"uppercase",fontFamily:font,marginBottom:"24px" }}>Start</button>
      )}
      {breatheActive && (
        <div style={{ marginBottom:"24px" }}>
          <div style={{ width:"100px",height:"100px",borderRadius:"50%",margin:"0 auto 16px",
            background:`${C.accent}22`,border:`2px solid ${C.accent}`,
            display:"flex",alignItems:"center",justifyContent:"center",
            animation:"breathe 1s ease-in-out infinite" }}>
            <p style={{ color:C.accent,fontSize:"11px",letterSpacing:"1px",fontStyle:"italic",margin:0,fontFamily:font }}>{phaseDuration[breathePhase]}</p>
          </div>
          <p style={{ color:C.textPrimary,fontSize:"18px",fontStyle:"italic",margin:"0 0 6px",fontFamily:font }}>{phaseLabel[breathePhase]}</p>
          <p style={{ color:C.textMuted,fontSize:"11px",fontStyle:"italic" }}>Cycle {breatheCount+1} of 4</p>
        </div>
      )}
      {!breatheActive && breatheCount >= 4 && (
        <div style={{ marginBottom:"24px" }}>
          <p style={{ fontSize:"28px",margin:"0 0 8px" }}>✓</p>
          <p style={{ color:C.sage,fontSize:"13px",fontStyle:"italic",lineHeight:"1.8" }}>4 cycles complete. Your body is slowing down.</p>
        </div>
      )}
      {(breatheCount >= 4 || (!breatheActive && breatheCount===0)) && (
        <button onClick={()=>setStep(3)} style={{ background:breatheCount>=4?C.accent:"transparent",
          border:`1.5px solid ${breatheCount>=4?C.accent:C.border}`,borderRadius:"4px",
          color:breatheCount>=4?"#fff":C.textMuted,fontFamily:font,fontSize:"10px",
          letterSpacing:"3px",textTransform:"uppercase",padding:"14px 28px",cursor:"pointer" }}>
          {breatheCount>=4?"Continue →":"Skip →"}
        </button>
      )}
    </div>,
    ...
    // Scripture
    <div key="scripture">
      <p style={{ color:C.accent,fontSize:"9px",letterSpacing:"3px",textTransform:"uppercase",fontStyle:"italic",margin:"0 0 16px" }}>Step 3 of 4 · Scripture</p>
      <h2 style={{ color:C.textPrimary,fontSize:"20px",fontWeight:"normal",margin:"0 0 20px" }}>A word for the night.</h2>
      <div style={{ background:`${C.accent}08`,border:`1px solid ${C.accent}22`,borderRadius:"12px",padding:"24px 20px",marginBottom:"20px",textAlign:"center" }}>
        <p style={{ color:C.textPrimary,fontSize:"17px",fontStyle:"italic",lineHeight:"1.9",margin:"0 0 12px",fontFamily:font }}>
          "{verse.text}"
        </p>
        <p style={{ color:C.accent,fontSize:"10px",letterSpacing:"2px",textTransform:"uppercase",margin:0 }}>{verse.ref}</p>
      </div>
      <p style={{ color:C.textSoft,fontSize:"12px",fontStyle:"italic",lineHeight:"1.8",margin:"0 0 24px",textAlign:"center" }}>
        Read it once. Breathe. Let it be the last real thing you think about tonight.
      </p>
      <button onClick={()=>setStep(4)} style={{ width:"100%",background:C.accent,border:"none",borderRadius:"4px",
        color:"#fff",fontFamily:font,fontSize:"10px",letterSpacing:"3px",textTransform:"uppercase",
        padding:"15px",cursor:"pointer" }}>Received it →</button>
    </div>,

    // Prayer
    <div key="prayer">
      <p style={{ color:C.accent,fontSize:"9px",letterSpacing:"3px",textTransform:"uppercase",fontStyle:"italic",margin:"0 0 16px" }}>Step 4 of 4 · Prayer</p>
      <h2 style={{ color:C.textPrimary,fontSize:"20px",fontWeight:"normal",margin:"0 0 10px" }}>Hand it to God.</h2>
      <p style={{ color:C.textSoft,fontSize:"13px",fontStyle:"italic",lineHeight:"1.8",margin:"0 0 20px" }}>
        You don't need to solve anything tonight. Whatever you wrote down, whatever is still unfinished — hand it over. He's already working on it while you sleep.
      </p>
      <div style={{ background:`${C.sage}08`,border:`1px solid ${C.sage}22`,borderRadius:"10px",padding:"18px",marginBottom:"24px" }}>
        <p style={{ color:C.textSoft,fontSize:"13px",fontStyle:"italic",lineHeight:"1.9",margin:0,fontFamily:font }}>
          "Lord, I give you tonight. What I couldn't finish, what I couldn't fix, what I'm still holding — it's yours. Give me rest. Restore me for tomorrow. Amen."
        </p>
      </div>
      <button onClick={()=>setStep(5)} style={{ width:"100%",background:C.sage,border:"none",borderRadius:"4px",
        color:"#fff",fontFamily:font,fontSize:"10px",letterSpacing:"3px",textTransform:"uppercase",
        padding:"15px",cursor:"pointer" }}>Amen →</button>
    </div>,

    // Done
    <div key="done" style={{ textAlign:"center", padding:"20px 0" }}>
      <p style={{ fontSize:"36px",margin:"0 0 14px" }}>🌙</p>
      <h2 style={{ color:C.textPrimary,fontSize:"22px",fontWeight:"normal",margin:"0 0 10px" }}>Rest well, {userName||"friend"}.</h2>
      <p style={{ color:C.textSoft,fontSize:"13px",fontStyle:"italic",lineHeight:"1.9",margin:"0 0 28px" }}>
        You showed up today. That matters. Put the phone down and sleep.
      </p>
      <button onClick={()=>setScreen("home")} style={{ background:"none",border:`1px solid ${C.border}`,
        borderRadius:"4px",color:C.textMuted,fontFamily:font,fontSize:"10px",letterSpacing:"3px",
        textTransform:"uppercase",padding:"14px 28px",cursor:"pointer" }}>Close</button>
    </div>,
  ];

  return (
    <div style={{ minHeight:"100vh", background:C.bgPrimary, fontFamily:font,
      padding:"40px 24px 100px", boxSizing:"border-box" }}>
      <div style={{ maxWidth:"480px", margin:"0 auto" }}>
        <button onClick={()=>setScreen("home")} style={{ background:"none",border:"none",
          cursor:"pointer",color:C.textMuted,fontSize:"20px",marginBottom:"24px" }}>←</button>
        {steps[step]}
      </div>
    </div>
  );
...
function SelahAppInner() {
  ...
  const [appScreen, setAppScreen] = useState(()=>{
    // ?new=true from install page — start onboarding for new user
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get("new") === "true") {
      // Only reset if they haven't already completed onboarding
      const existingScreen = has("appScreen") ? saved.appScreen : null;
      if (!existingScreen || existingScreen === "landing") return "landing";
      // Existing user — just go to main
...
useEffect(() => {
  ...
  const [gratitudeLog, setGratitudeLog] = useState(has("gratitudeLog") ? saved.gratitudeLog : []);
  ...
  useEffect(() => {
    ...
    fetch("/api/auth", {
      ...
      if (d.success) {
        ...
        // Server is authoritative for trialStart
  ...
  const renderMain = () => {
    ...
    switch(screen) {
      ...
      case "winddown": {
        ...
        return <WindDownScreen C={SC} font={font} setScreen={setScreen}
          userName={userName} faithLevel={faithLevel} tier={effectiveTier}
          isTrialActive={isTrialActive} onUpgrade={()=>setShowSub(true)}/>;
      }
      case "letters":
        ...
        <SettingsScreen C={SC} font={font} setScreen={setScreen}
          ...
          onLogin={async(email, code)=>{
            ...
            if(d.success){
              ...
              // Server is authoritative for trialStart — prevents localStorage clearing abuse
              if(d.trialStart) setTrialStart(d.trialStart);
  ...
  if(appScreen!=="main") {
    const flowContent = (() => {
      ...
      if(appScreen==="splash") return <SplashScreen onDone={()=>setAppScreen("emailgate")} C={C} font={font}/>;
      if(appScreen==="emailgate") return (
        <EmailGateScreen C={C} font={font}
          onVerified={async({ token, email: gateEmail, trialStart: serverTrialStart })=>{
            // Lock in auth state from gate
            setAuthToken(token);
            setUserEmail(gateEmail);
            localStorage.setItem("selah_auth_token", token);
            // Server is authoritative for trialStart
            if(serverTrialStart) setTrialStart(serverTrialStart);
            // Send welcome email
            fetch("/api/emails",{method:"POST",headers:{"Content-Type":"application/json"},
              body:JSON.stringify({action:"welcome",email:gateEmail,name:userName||"friend"})}).catch(()=>{});
            // Try to restore cloud data (returning user)
            try{
